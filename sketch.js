const zine = {
  ...
  description: "this is the description for how to listen to a samosa",
  author: "roguentropy"
}
...
// Commented out color changing variables for Page One - REMOVED AS PER PREVIOUS INSTRUCTION
// let neonColors = [
//   [0, 255, 255], // Neon Cyan
...
// ];
// let currentColorIndex = 0; // To keep track of the current background color
// let lastColorChangeTime = 0; // To store the time of the last color change (in milliseconds)
// const CHANGE_INTERVAL = 5000; // 5 seconds in milliseconds
...
function preload() {
  ...
  img8 = loadImage("mughal_bgremove.png"); // Added img8
  ...
  img12 = loadImage("saraswati.jpg"); // Added img12
  img13 = loadImage("Ravi_Varma-Ravana_Sita_Jathayu.jpg"); // Added img13
  ...
  // Load the samosa image for Page One background
...
function setupPage() {
  ...
  // Initialize bird positions for Page One (adapted from snippet's setup)
  // Use one.width and one.height for positioning relative to the page
  ...
  // lastColorChangeTime = millis(); // Initialize the time tracker (millis() is global p5 function) - COMMENTED OUT
...
function drawPage() {
  ...
  for (let x = 0; x < one.width; x += gridSize) {
    for (let y = 0; y < one.height; y += gridSize) {
      ...
      // Color Gradient for tinting the image - REMOVED AS PER YOUR INSTRUCTION
      // let r = one.map(x, 0, one.width, 50, 255);
      // let g = one.map(y, 0, one.height, 0, 150);
      // let b = one.map(x, 0, one.width, 255, 50);
      // one.tint(r, g, b, 200);
  ...
  // m += 0.5; // Removed global 'm' update as it's unused for animation
  ...
  // Bird 5: Random Movement - Reverted to original birdScale
  ...
  two.background(155); // Set a background color for Page Two (e.g., white)
  two.noStroke(); // No stroke for the images

  // Updated array of images to choose from, including img10-13
  let randomImages = [img3, img8, img9, img10, img11, img12, img13];
  let numRandomImages = 5; // Reduced number of random images to display for slower animation

  for (let i = 0; i < numRandomImages; i++) {
    let imgToDraw = two.random(randomImages); // Randomly select an image
    let imgWidth = imgToDraw.width * 0.2; // Adjust size of random images
  ...
  // You can still keep the text on the left side if desired
  two.push();
  two.textSize(30);
  two.textFont("Modak"); // Ensure "Modak" font is linked in index.html if you want to use it
  two.fill("green");
  two.textAlign(two.LEFT, two.TOP);
  two.text("hello", two.width * 0.05, two.height * 0.1);
  two.text("goodbye", two.width * 0.05, two.height * 0.2);
  two.pop();
  ...
  three.textSize(40)
  three.fill(40, 60)
  three.textFont("Boldonse")
  three.glitchLayout(["@", " ", "*"], 5, 30, 20, true)
